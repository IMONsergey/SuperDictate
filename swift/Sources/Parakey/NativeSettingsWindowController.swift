...
/// remains authoritative for state and mutations. The controller itself never
/// reads/writes Settings, service state, updater state or TCC.
@MainActor
final class NativeSettingsWindowController: NSObject, NSWindowDelegate {
    ...
    init(
        ...
        model = SuperDictateSettingsModel(snapshot: initialSnapshot, language: language)
    ...
    func refresh(snapshot: SuperDictateSettingsSnapshot,
                 language: SuperDictateInterfaceLanguage) {
        if model.snapshot != snapshot { model.snapshot = snapshot }
        if model.language != language { model.language = language }
    }
