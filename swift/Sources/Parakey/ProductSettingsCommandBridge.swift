...
///
/// UI-only navigation/updater/service/permission actions remain in the visible
/// control-panel process. Dictation behavior and transcript-history persistence
/// cross this same-bundle trust boundary so there is still one runtime owner.
...
final class ProductSettingsCommandObserver: NSObject {
    ...
    init(
        onCommand: @escaping @MainActor (ProductAgentSettingsCommand) -> Void
    ) {
    ...
    /// Cross-process Settings writes are accepted only from the foreground copy
    /// of the exact same installed app. Bundle ID alone is insufficient because
    /// another bundle can claim it; bundle URL alone is insufficient without the
    /// expected ID. Requiring the sender to be frontmost also prevents a hidden
    /// background process from mutating privacy state through the notification.
