...
enum MemorySystemSampleBufferCopy {
    static func makeBlock(
        ...
        guard asbd.mFormatID == kAudioFormatLinearPCM else {
            throw MemorySystemSampleBufferCopyError.unsupportedFormatID(asbd.mFormatID)
        }
        guard asbd.mBitsPerChannel == 32 else {
            throw MemorySystemSampleBufferCopyError.unsupportedBitsPerChannel(asbd.mBitsPerChannel)
        }
        let requiredFlags = AudioFormatFlags(kAudioFormatFlagIsFloat)
        guard asbd.mFormatFlags & requiredFlags == requiredFlags else {
            throw MemorySystemSampleBufferCopyError.unsupportedPCMFlags(asbd.mFormatFlags)
        }
        ...
        guard channelCount == 1 || channelCount == 2 else {
            throw MemorySystemSampleBufferCopyError.invalidChannelCount(channelCount)
        }
    ...
    private static func copiedFloatChannels(
        ...
        guard queryStatus == noErr,
            ...
            throw MemorySystemSampleBufferCopyError.audioBufferListQueryFailed(queryStatus)
        }
        ...
        guard readStatus == noErr else {
            throw MemorySystemSampleBufferCopyError.audioBufferListReadFailed(readStatus)
        }
        ...
        if isNonInterleaved {
            ...
            for channelIndex in 0..<channelCount {
                ...
                result.append(Array(UnsafeBufferPointer(start: pointer, count: frameCount)))
            }
            return result
        }
        ...
        var result = Array(repeating: [Float](), count: channelCount)
        for channelIndex in 0..<channelCount {
        ...
        for frameIndex in 0..<frameCount {
            ...
            for channelIndex in 0..<channelCount {
                result[channelIndex].append(interleaved[base + channelIndex])
            }
        }
        return result
    }
