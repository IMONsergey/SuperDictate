...
public struct SuperDictateMemoryRecoveryBatchResult: Equatable, Sendable {
    ...
    /// Deliberately ID-only. Startup diagnostics must not expose private paths or
    /// source content just because one package was unreadable.
...
public actor SuperDictateMemoryPackageRecovery {
    ...
    public func recover(recordingID: UUID) async throws -> SuperDictateMemoryRecoveryResult {
        ...
        var needsAttention = manifest.state == .needsAttention
        ...
        // Verify everything the manifest already claims as durable. If a crash
        // happened after manifest append but before the committed journal marker,
        // close the prepared event with the manifest's already-verified SHA.
            ...
            do {
                ...
                if let event = latestJournalByPath[chunk.relativePath] {
                    ...
                    if event.kind == .chunkPrepared {
                        do {
                            let committed = try committedEvent(from: event, sha256: chunk.sha256)
        ...
        // Reconcile source files that are not in the manifest.
        for source in SuperDictateMemoryAudioSource.allCases {
            ...
            for url in try sourceFiles(sourceDirectory) {
                ...
                do {
                    ...
                    if event.kind == .chunkPrepared {
                        do {
                            let committed = try committedEvent(
                                from: event,
                                sha256: inspection.sha256
                            )
                            try await store.appendRecoveryEvent(committed)
        ...
        // A durable journal entry proves that bytes crossed an earlier capture
        // boundary. If no manifest/partial/final object accounts for it now, the
        // source is missing rather than silently forgotten.
    ...
    /// One unreadable package must not block recovery of the rest of the local
    /// Memory Library. Callers get ID-only failures and can surface them as
    /// attention items without leaking private file paths.
...
private extension JSONSuperDictateMemoryPackageStore {
    /// Integrity-only mutation. It may repair a verified orphan into a package
    /// that is already `.ready`, but external capture adapters cannot call it.
    @discardableResult
