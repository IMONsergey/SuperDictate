...
public actor SuperDictateMemoryPackageRecovery {
    ...
    private func validateDirectoryNoFollow(_ directory: URL, normalizePermissions: Bool = false) throws {
        var st = stat()
    ...
    private func synchronizeDirectory(_ directory: URL) throws {
        ...
        guard fd >= 0 else {
            if errno == ELOOP { throw SuperDictateMemoryRecoveryError.unsafeFileType(directory.lastPathComponent) }
            throw SuperDictateMemoryRecoveryError.posix(errno)
        }
        defer { _ = Darwin.close(fd) }
        guard Darwin.fsync(fd) == 0 else { throw SuperDictateMemoryRecoveryError.posix(errno) }
    }
    ...
    private func inspectCAF(_ url: URL) throws -> (byteLength: Int64, sha256: String) {
        ...
        guard fd >= 0 else {
            if errno == ELOOP { throw SuperDictateMemoryRecoveryError.unsafeFileType(url.lastPathComponent) }
            throw SuperDictateMemoryRecoveryError.posix(errno)
        }
        defer { _ = Darwin.close(fd) }

        var st = stat()
        guard Darwin.fstat(fd, &st) == 0 else { throw SuperDictateMemoryRecoveryError.posix(errno) }
        ...
        guard byteLength >= 4 else { throw SuperDictateMemoryRecoveryError.invalidContainer(url.lastPathComponent) }
