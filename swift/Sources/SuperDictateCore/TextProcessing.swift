...
/// Pure post-ASR text processing shared by instant dictation and memory capture.
///
/// Processing order is a compatibility invariant:
...
///
/// Explicit corrections run before filler removal so user intent always wins.
        ...
        for correction in active {
            ...
            regex.enumerateMatches(in: text, range: fullRange) { match, _, _ in
                guard let range = match?.range,
                      ...
                      !matches.contains(where: { NSIntersectionRange($0.range, range).length > 0 }) else {
                    return
                }
