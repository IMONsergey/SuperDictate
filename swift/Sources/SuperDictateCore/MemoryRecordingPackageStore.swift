...
/// Private package-level source of truth for long-form Memory Capture recordings.
///
/// The Library index is intentionally not authoritative for source audio. Each
/// Memory Capture owns a private package keyed by the stable recording UUID:
///
/// `memory-recordings/<recording-id>/manifest.json`
/// `memory-recordings/<recording-id>/audio/microphone/`
/// `memory-recordings/<recording-id>/audio/system/`
/// `memory-recordings/<recording-id>/audio/imported/`
/// `memory-recordings/<recording-id>/quarantine/`
///
...
public actor JSONSuperDictateMemoryPackageStore {
    ...
    public func createPackage(
        ...
        do {
            try Self.createPrivateDirectoryExclusively(
                packageURL.appendingPathComponent("audio", isDirectory: true)
            )
            ...
            // Package creation is all-or-nothing before any source audio exists.
            // Once adapters start writing chunks this store exposes no recursive
            // deletion API; recovery/quarantine policy owns those bytes.
    ...
    public func loadManifest(
        ...
        try Self.validatePrivateDirectory(packageURL)
    ...
    public func existingRecordingIDs() throws -> [UUID] {
        ...
        for name in names {
            ...
            do {
                ...
                // A symlink or non-directory pretending to be a package is not a
                // recording and must never become a recovery target.
                continue
            }
        }
    ...
    private static func atomicPrivateWrite(
        ...
        if FileManager.default.fileExists(atPath: targetURL.path) {
            ...
            var st = stat()
            let ok = Darwin.fstat(fd, &st) == 0
            let savedErrno = errno
            _ = Darwin.close(fd)
            guard ok else { throw SuperDictateMemoryPackageStoreError.posix(savedErrno) }
            guard (st.st_mode & S_IFMT) == S_IFREG,
                  st.st_nlink == 1 else {
                throw SuperDictateMemoryPackageStoreError.unsafeFileType
            }
