...
public enum SuperDictateMemoryRecoveryEventKind: String, Codable, Sendable {
    /// Metadata is durably journaled before the source file crosses the final
    /// rename boundary. A crash after this point can reconstruct the descriptor
    /// without inventing timing/audio-format facts.
    case chunkPrepared = "chunk_prepared"
    /// The corresponding descriptor is already present in manifest.json.
...
public struct SuperDictateMemoryRecoveryEvent: Codable, Equatable, Sendable, Identifiable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        try self.init(
            ...
            kind: container.decode(SuperDictateMemoryRecoveryEventKind.self, forKey: .kind),
            recordingID: container.decode(UUID.self, forKey: .recordingID),
            chunkID: container.decode(UUID.self, forKey: .chunkID),
...
public struct SuperDictateMemoryRecoveryJournalSnapshot: Equatable, Sendable {
    ...
    /// A process may die while the final JSONL append is in progress. Only one
    /// incomplete tail line is tolerated; corruption in the middle is fatal.
    public var ignoredPartialTailLine: Bool

    public init(
        events: [SuperDictateMemoryRecoveryEvent],
        ignoredPartialTailLine: Bool = false
    ) {
        self.events = events
...
public extension JSONSuperDictateMemoryPackageStore {
    ...
    private static func readRecoveryJournalBytes(
        _ url: URL,
        maximumBytes: Int
    ) throws -> Data {
