...
public struct SuperDictateMemoryRecoveryEvent: Codable, Equatable, Sendable, Identifiable {
    ...
    private enum CodingKeys: String, CodingKey {
        ...
        case container, codec, sampleRate, channelCount, byteLength, createdAt
    }
...
public extension JSONSuperDictateMemoryPackageStore {
    ...
    func appendRecoveryEvent(_ event: SuperDictateMemoryRecoveryEvent) throws {
        ...
        let fd = Darwin.open(url.path, O_WRONLY | O_CREAT | O_APPEND | O_CLOEXEC | O_NOFOLLOW, mode_t(0o600))
        guard fd >= 0 else {
        ...
        var st = stat()
        guard Darwin.fstat(fd, &st) == 0 else { throw SuperDictateMemoryRecoveryJournalError.posix(errno) }
        ...
        guard st.st_size + off_t(line.count) <= off_t(Self.maximumRecoveryJournalBytes) else {
            throw SuperDictateMemoryRecoveryJournalError.fileTooLarge(Int(st.st_size) + line.count, Self.maximumRecoveryJournalBytes)
        }
        guard Darwin.fchmod(fd, mode_t(0o600)) == 0 else { throw SuperDictateMemoryRecoveryJournalError.posix(errno) }
        try Self.writeRecoveryJournalBytes(line, fd: fd)
        guard Darwin.fsync(fd) == 0 else { throw SuperDictateMemoryRecoveryJournalError.posix(errno) }
    }

    func recoveryJournalSnapshot(recordingID: UUID) throws -> SuperDictateMemoryRecoveryJournalSnapshot {
        ...
        let data = try Self.readRecoveryJournalBytes(url, maximumBytes: Self.maximumRecoveryJournalBytes)
        guard !data.isEmpty else { return SuperDictateMemoryRecoveryJournalSnapshot(events: []) }
        ...
        for (index, line) in lines.enumerated() {
            ...
            do {
                let event = try decoder.decode(SuperDictateMemoryRecoveryEvent.self, from: Data(line))
                guard event.recordingID == recordingID else { throw SuperDictateMemoryRecoveryJournalError.invalidMetadata }
                events.append(event)
            } catch {
                let isLastPhysicalLine = index == lines.count - 1
                let fileHasTrailingNewline = data.last == UInt8(ascii: "\n")
                if isLastPhysicalLine && !fileHasTrailingNewline {
                    ignoredTail = true
                    break
                }
                if let typed = error as? SuperDictateMemoryRecoveryJournalError { throw typed }
                throw SuperDictateMemoryRecoveryJournalError.corruptLine(index + 1)
            }
        }
        return SuperDictateMemoryRecoveryJournalSnapshot(events: events, ignoredPartialTailLine: ignoredTail)
    }

    func recoveryJournalURL(recordingID: UUID) -> URL {
        packageURL(recordingID: recordingID).appendingPathComponent("recovery.jsonl", isDirectory: false)
    }
    ...
    private static func writeRecoveryJournalBytes(_ data: Data, fd: Int32) throws {
        try data.withUnsafeBytes { raw in
            ...
            while offset < raw.count {
                ...
                guard result > 0 else { throw SuperDictateMemoryRecoveryJournalError.posix(EIO) }
                offset += result
            }
        }
    }
    ...
    private static func readRecoveryJournalBytes(_ url: URL, maximumBytes: Int) throws -> Data {
        ...
        var st = stat()
        guard Darwin.fstat(fd, &st) == 0 else { throw SuperDictateMemoryRecoveryJournalError.posix(errno) }
        guard (st.st_mode & S_IFMT) == S_IFREG, st.st_nlink == 1 else { throw SuperDictateMemoryRecoveryJournalError.unsafeFileType }
        guard st.st_size >= 0, st.st_size <= off_t(maximumBytes) else {
            throw SuperDictateMemoryRecoveryJournalError.fileTooLarge(Int(max(0, st.st_size)), maximumBytes)
        }
        var data = Data()
        ...
        while true {
            ...
            guard data.count + count <= maximumBytes else {
                throw SuperDictateMemoryRecoveryJournalError.fileTooLarge(data.count + count, maximumBytes)
            }
