...
public struct SuperDictateMemoryRecoveryEvent: Codable, Equatable, Sendable, Identifiable {
    ...
    /// Nil before the rename boundary. A committed event must cryptographically
    /// bind the journal record to the exact finalized source bytes.
...
public extension JSONSuperDictateMemoryPackageStore {
    ...
    func appendRecoveryEvent(_ event: SuperDictateMemoryRecoveryEvent) throws {
        ...
        // Validate the already-durable prefix before extending it. This is not
        // just input validation: it prevents a later prepared event from
        // downgrading a committed SHA-bound chunk to weaker recovery trust.
        let current = try recoveryJournalSnapshot(recordingID: event.recordingID)
        try Self.validateJournalTransition(existing: current.events, next: event)
    ...
    func recoveryJournalSnapshot(
        ...
        for (index, line) in lines.enumerated() {
            ...
            do {
                ...
                try Self.validateJournalTransition(existing: events, next: event)
                events.append(event)
            } catch {
    ...
    private static func validateJournalTransition(
