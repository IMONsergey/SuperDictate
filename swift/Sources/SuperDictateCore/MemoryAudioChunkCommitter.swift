...
/// Encoding-neutral durability transaction for one completed Memory audio chunk.
///
/// Audio adapters write and close the exact `.partial` CAF returned by this actor.
/// Core then owns the trust boundary:
///
/// temporary CAF -> fsync -> prepared journal -> atomic rename -> directory fsync
/// -> SHA-256 -> actor-isolated manifest append -> checksum-bound committed event.
///
/// Once rename succeeds source bytes are never deleted by this transaction. A
/// later failure returns `recoveryRequired` so package recovery can prove and
/// reattach or quarantine the orphan.
public actor SuperDictateMemoryAudioChunkCommitter {
    ...
    public func commitChunk(
        ...
        do {
            ...
            // The manifest is already authoritative after its atomic+fsynced
            // save. A missing committed marker is recoverable; do not turn a
            // successfully persisted source into a failed dictation solely
            // because this advisory closing append failed.
            if let committed = try? SuperDictateMemoryRecoveryEvent(
                kind: .chunkCommitted,
                recordingID: recordingID,
                chunkID: chunkID,
                source: source,
