...
extension ProductStateTests {
    ...
    func testMemoryRecoveryJournalRoundTripsPreparedAndCommittedEvents() async throws {
        ...
        let prepared = try recoveryEvent(kind: .chunkPrepared, recordingID: recordingID, chunkID: chunkID)
        let committed = try recoveryEvent(kind: .chunkCommitted, recordingID: recordingID, chunkID: chunkID)
    ...
    func testMemoryRecoveryJournalRejectsCorruptionBeforeTail() async throws {
        ...
        try await store.appendRecoveryEvent(
            recoveryEvent(kind: .chunkPrepared, recordingID: recordingID, chunkID: UUID())
        )
        let url = await store.recoveryJournalURL(recordingID: recordingID)
        let handle = try FileHandle(forWritingTo: url)
        try handle.seekToEnd()
        ...
        try handle.close()
        try await store.appendRecoveryEvent(
            recoveryEvent(kind: .chunkPrepared, recordingID: recordingID, chunkID: UUID(), sequence: 1)
        )
    ...
    func testMemoryRecoveryEventRejectsPathThatDoesNotMatchSourceAndChunkIdentity() {
        let bad = "audio/microphone/wrong.caf"
        XCTAssertThrowsError(
            ...
            XCTAssertEqual(error as? SuperDictateMemoryRecoveryJournalError, .invalidRelativePath(bad))
        }
    }
}
