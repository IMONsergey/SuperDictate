...
extension ProductStateTests {
    ...
    func testSuccessfulMemoryChunkWritesPreparedThenCommittedRecoveryMarkers() async throws {
        ...
        let temp = try await committer.temporaryChunkURL(recordingID: recordingID, source: .system, sequence: 0, chunkID: chunkID)
        try journalCAFBytes("committed").write(to: temp)
        _ = try await committer.commitChunk(
            recordingID: recordingID, source: .system, sequence: 0, chunkID: chunkID,
            temporaryURL: temp, sessionStartMilliseconds: 250, sessionEndMilliseconds: 1_250,
