...
extension ProductStateTests {
    private func isolationV4CAF(_ payload: String) -> Data {
        var data = Data("caff".utf8)
        data.append(Data(payload.utf8))
        return data
    }

    private func isolationV4SHA(_ data: Data) -> String {
        SHA256.hash(data: data).map { String(format: "%02x", $0) }.joined()
    }

    func testQuarantineSymlinkAbortsBeforeMovingSourceBytes() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("SuperDictateRecoverV4Quarantine-\(UUID().uuidString)", isDirectory: true)
        ...
        try isolationV4CAF("undocumented").write(to: sourceURL)
    ...
    func testBatchRecoveryContinuesPastUnreadablePackage() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("SuperDictateRecoverV4Batch-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let store = try JSONSuperDictateMemoryPackageStore(rootDirectory: root)
    ...
    func testRecoveryClosesPreparedMarkerWhenManifestAlreadyCommittedSource() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("SuperDictateRecoverV4ClosePrepared-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let store = try JSONSuperDictateMemoryPackageStore(rootDirectory: root)
        let recordingID = UUID()
        let chunkID = UUID()
        ...
        let bytes = isolationV4CAF("manifest-won")
        let relativePath = SuperDictateMemoryAudioChunk.canonicalRelativePath(
            source: .microphone,
            sequence: 0,
            chunkID: chunkID
    ...
    func testJournalMetadataConflictDoesNotDestroyValidManifestSource() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("SuperDictateRecoverV4JournalConflict-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let store = try JSONSuperDictateMemoryPackageStore(rootDirectory: root)
        let recordingID = UUID()
        let chunkID = UUID()
        ...
        let bytes = isolationV4CAF("valid-source")
        let relativePath = SuperDictateMemoryAudioChunk.canonicalRelativePath(
            source: .microphone,
            sequence: 0,
            chunkID: chunkID
