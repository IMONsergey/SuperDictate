...
extension ProductStateTests {
    ...
    func testMemoryRecoveryQuarantinesUndocumentedFinalizedOrphan() async throws {
        ...
        let relativePath = SuperDictateMemoryAudioChunk.canonicalRelativePath(source: .system, sequence: 0, chunkID: chunkID)
        let finalURL = sourceDirectory.appendingPathComponent(URL(fileURLWithPath: relativePath).lastPathComponent)
    ...
    func testPreparedJournalEventWithNoSourceBytesIsReportedMissing() async throws {
        ...
        try await store.appendRecoveryEvent(
            preparedRecoveryEvent(recordingID: recordingID, chunkID: chunkID, byteLength: 100)
        )
    ...
    func testReadyPackageChecksumCorruptionIsQuarantinedAndDowngraded() async throws {
        ...
        let committer = SuperDictateMemoryAudioChunkCommitter(store: store)
        let chunkID = UUID()
        let partial = try await committer.temporaryChunkURL(recordingID: recordingID, source: .microphone, sequence: 0, chunkID: chunkID)
        try recoveryCAFBytes("original").write(to: partial)
        let descriptor = try await committer.commitChunk(
            recordingID: recordingID, source: .microphone, sequence: 0, chunkID: chunkID,
            temporaryURL: partial, sessionStartMilliseconds: 0, sessionEndMilliseconds: 100,
    ...
    func testReadyPackageMissingManifestSourceIsReported() async throws {
        ...
        _ = try await store.markReady(recordingID: recordingID)
        let packageURL = await store.packageURL(recordingID: recordingID)
        try FileManager.default.removeItem(at: packageURL.appendingPathComponent(descriptor.relativePath))
