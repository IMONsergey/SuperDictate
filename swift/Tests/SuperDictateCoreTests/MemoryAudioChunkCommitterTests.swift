...
extension ProductStateTests {
    ...
    func testMemoryChunkCommitFinalizesHashesAndAppendsManifest() async throws {
        ...
        XCTAssertEqual(try XCTUnwrap(maybeManifest).chunks, [descriptor])
    }
    ...
    func testOversizedChunkStaysPartial() async throws {
        ...
        let recordingID = UUID()
        _ = try await store.createPackage(recordingID: recordingID)
        let committer = SuperDictateMemoryAudioChunkCommitter(
            store: store,
    ...
    func testReadyPackageRejectsNewChunkBeforeRename() async throws {
        ...
        do {
            ...
            XCTAssertEqual(error as? SuperDictateMemoryManifestError, .chunkAppendNotAllowed(.ready))
        }
        XCTAssertTrue(FileManager.default.fileExists(atPath: temporaryURL.path))
    }
    ...
    func testPostRenameManifestFailurePreservesOrphanForRecovery() async throws {
        ...
        let firstID = UUID()
        let firstTemp = try await committer.temporaryChunkURL(recordingID: recordingID, source: .microphone, sequence: 0, chunkID: firstID)
        try cafTestBytes("first").write(to: firstTemp)
        ...
        let orphanID = UUID()
        let orphanTemp = try await committer.temporaryChunkURL(recordingID: recordingID, source: .microphone, sequence: 0, chunkID: orphanID)
        try cafTestBytes("orphan").write(to: orphanTemp)
        do {
            _ = try await committer.commitChunk(
        ...
        let maybeManifest = try await store.loadManifest(recordingID: recordingID)
        XCTAssertEqual(try XCTUnwrap(maybeManifest).chunks, [first])
    }
}
